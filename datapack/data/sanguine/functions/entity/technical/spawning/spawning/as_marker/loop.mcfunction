...
execute at @s if entity @a[distance=..14,gamemode=!spectator] run scoreboard players set #valid sanguine.dummy 0
...
execute unless score #spawned sanguine.dummy matches 1.. at @s align xyz positioned ~0.5 ~ ~0.5 if score #valid sanguine.dummy matches 1 run function sanguine:entity/technical/spawning/spawning/as_marker/finish
execute if score #total_checks sanguine.dummy matches 10.. run kill @s
