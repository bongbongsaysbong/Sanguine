...
execute as @e[distance=..2,type=#nucleus:preset/can_take_damage/mobs,team=!sanguine.flesh_tuner,tag=!smithed.strict,tag=!smithed.block,nbt=!{Invulnerable:1b}] run function sanguine:item/butcher_skull/as_mob
execute as @a[distance=..2,tag=!sanguine.butcher_skull_damager,team=!sanguine.flesh_tuner] run function sanguine:entity/player/damage/butcher_skull
