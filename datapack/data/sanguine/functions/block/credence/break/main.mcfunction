...
execute store success score #drop_item sanguine.dummy run kill @e[type=item,tag=!smithed.entity,distance=..5,nbt={Item:{id:"minecraft:barrel",tag:{display:{Name:'{"color":"white","font":"sanguine:credence","translate":"block.sanguine.credence.gui"}'}}}}]
...
execute if entity @s[tag=sanguine.credence.has_output_slot] run function sanguine:block/credence/break/drop_item/main
